...
class Admin extends BaseController
{
    ...
    public function markAsComplete($report_id){
        ...
        $buyer_phone = '';
        $seller_phone = '';
        $updateSellerSaccoShare = '';
        $updateBuyerSaccoShare = '';
        $seller_message = '';
        $buyer_message = '';

        try {
            $transaction_report = $this->adminModel->getReport($sacco_id, $report_id);
            $seller_user_id = $transaction_report[0]['seller_user_id'];
            $buyer_user_id = $transaction_report[0]['buyer_user_id'];
            $shares_on_sale = $transaction_report[0]['shares_on_sale'];
        ...
        try{
            $sacco_shares = $this->adminModel->getSaccoShares($seller_user_id, $sacco_id);
            $shares_balance = $sacco_shares[0]['shares_amount'] - $shares_on_sale;
        }catch(\CodeIgniter\Database\Exceptions\DataBaseException $e){
            $this->adminModel->insertError($e->getMessage());
        }

        try{
            $updateSellerSaccoShare = $this->adminModel->updateSellerSacoShare($seller_user_id, $shares_balance);
        }catch(\CodeIgniter\Database\Exceptions\DataBaseException $e){
            $this->adminModel->insertError($e->getMessage());
        }
        try{
            $updateBuyerSaccoShare = $this->adminModel->updateBuyerSaccoShare($buyer_user_id, $shares_on_sale);
        }catch(\CodeIgniter\Database\Exceptions\DataBaseException $e){
            $this->adminModel->insertError($e->getMessage());
        }
        try{
        ...
        $seller_message = "Congratulations " . $seller_fname . ", your shares has been sold successfully, please wait for payment within 24 hours";
        $buyer_message = "Congratulations " . $buyer_fname . ", you have successfully bought shares from " . $seller_fname . ", your shares has been added to your account";
        $seller_message = $this->text_msg($seller_phone, $seller_message);
        $buyer_message = $this->text_msg($buyer_phone, $buyer_message);

        if($updateSellerSaccoShare && $updateBuyerSaccoShare && $seller_message && $buyer_message){
            session()->setFlashdata('success', 'Transaction is completed successfully');
            return redirect()->to(base_url('admin/reports'));
        }else{
            session()->setFlashdata('fail', 'Not complete, please check the logs for more information');
            return redirect()->to(base_url('admin/reports'));
        }
    }
    ...
    public function newUserPost(){
        if($this->request->getMethod() == 'post'){
            ...
            if($this->validate($rules)){
                session()->setFlashdata('fail', 'Please fill all the fields');
                return redirect()->back()->to(base_url('admin/new_user'));
            }else{
                ...
                if($this->adminModel->saveUser($data)) {
                    if($sendEmail){
                        return redirect()->to('admin/new_user')->with('success', 'The login password has been sent to  ' . $fname .' email');
                    }else{
                        return redirect()->to('admin/new_user')->with('fail', 'There was an error sending the password');
                    }
                }else{
                    session()->setFlashdata('fail', 'An error occurred, please try again');
                    return redirect()->to(base_url('admin/new_user'));
                }
            }
        }
    }
